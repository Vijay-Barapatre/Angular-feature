...
## 📋 Table of Contents
- [🎯 What are Async Guards?](#what-are-async-guards)
- [🔄 Return Types](#return-types)
- [💡 Real-World Use Cases](#real-world-use-cases)
- [📝 Implementation](#implementation)
  - [Basic Async Guard](#basic-async-guard)
  - [With Redirect on Failure](#with-redirect-on-failure)
...
```
┌─────────────────────────────────────────────────────────────┐
│  ASYNC GUARDS: WAITING FOR API RESPONSE                     │
│                                                             │
...
│   │    map(response => response.allowed),                 │ │
│   │    timeout(5000),  // ⏱️ Don't wait forever!          │ │
│   │    catchError(() => of(false))  // 🛡️ Handle errors! │ │
│   │  );                                                   │ │
│   └───────────────────────────────────────────────────────┘ │
│          │                                                  │
│          │  🔄 Angular WAITS for Observable to emit         │
│          │                                                  │
│          ▼                                                  │
│   ┌───────────────────────────────────────────────────────┐ │
│   │  API RESPONSE                                         │ │
│   │                                                       │ │
│   │  Case 1: API returns { allowed: true }                │ │
│   │    → Guard returns true → ✅ Navigate                 │ │
│   │                                                       │ │
│   │  Case 2: API returns { allowed: false }               │ │
│   │    → Guard returns false → 🛑 Block                   │ │
│   │                                                       │ │
│   │  Case 3: API times out or errors                      │ │
│   │    → catchError kicks in → of(false) → 🛑 Block       │ │
│   │                                                       │ │
│   │  Case 4: Return UrlTree for redirect                  │ │
│   │    → router.createUrlTree(['/unauthorized']) → 🔄     │ │
│   └───────────────────────────────────────────────────────┘ │
│                                                             │
│   BEST PRACTICES:                                           │
│   ┌───────────────────────────────────────────────────────┐ │
│   │ ✅ Always use catchError() - network fails happen     │ │
│   │ ✅ Add timeout() - don't hang indefinitely            │ │
│   │ ✅ Cache results - avoid repeated API calls           │ │
│   │ ✅ Show loading UI - user knows we're checking        │ │
│   └───────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
```

> **Key Takeaway**: Async guards let you call APIs before navigation. Always handle errors and timeouts!
