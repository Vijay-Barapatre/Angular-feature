...
## 📋 Table of Contents
- [1. 🔍 Pure vs Impure](#1--pure-vs-impure)
- [2. 🚀 Examples](#2--examples)
  - [Pure Pipe (Default)](#pure-pipe-default)
  - [Impure Pipe](#impure-pipe)
  - [📦 Data Flow Summary (Visual Box Diagram)](#data-flow-summary-visual-box-diagram)
- [3. ❓ Interview Questions](#3--interview-questions)
  - [Basic Questions](#basic-questions)
    - [Q1: Why doesn't my pipe update when I push to an array?](#q1-why-doesnt-my-pipe-update-when-i-push-to-an-array)
    - [Q2: Why are impure pipes bad for performance?](#q2-why-are-impure-pipes-bad-for-performance)
    - [Q3: When to use impure pipes?](#q3-when-to-use-impure-pipes)
  - [Scenario-Based Questions](#scenario-based-questions)
    - [Scenario: Filter Pipe Not Updating](#scenario-filter-pipe-not-updating)
- [🏎️ Sports Car vs Bus Analogy (Easy to Remember!)](#sports-car-vs-bus-analogy-easy-to-remember)
...
```
┌─────────────────────────────────────────────────────────────┐
│  PURE vs IMPURE: WHEN DOES PIPE RUN?                        │
│                                                             │
│   PURE PIPE (default) - Memoized:                           │
│   ┌───────────────────────────────────────────────────────┐ │
│   │ @Pipe({ pure: true })  // default                     │ │
│   │                                                       │ │
│   │ items.push(new);     → ❌ Pipe DOESN'T run (same ref) │ │
│   │ items[0].name = 'x'; → ❌ Pipe DOESN'T run (mutation) │ │
│   │ items = [...items];  → ✅ Pipe RUNS (new reference)   │ │
│   │                                                       │ │
│   │ 🏎️ Like sports car: only starts for NEW destination   │ │
│   └───────────────────────────────────────────────────────┘ │
│                                                             │
│   IMPURE PIPE - Always runs:                                │
│   ┌───────────────────────────────────────────────────────┐ │
│   │ @Pipe({ pure: false })                                │ │
│   │                                                       │ │
│   │ User types          → ✅ Pipe runs                    │ │
│   │ Button clicked      → ✅ Pipe runs                    │ │
│   │ Mouse moved         → ✅ Pipe runs                    │ │
│   │ ANYTHING happens    → ✅ Pipe runs (CD cycle)         │ │
│   │                                                       │ │
│   │ 🚌 Like city bus: runs every 10 seconds no matter what│ │
│   └───────────────────────────────────────────────────────┘ │
│                                                             │
│   RECOMMENDATION:                                           │
│   ┌───────────────────────────────────────────────────────┐ │
│   │ Use PURE (default) unless you MUST detect mutations   │ │
│   │ Use new references: items = [...items, newItem]       │ │
│   └───────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
```

> **Key Takeaway**: Pure = efficient (cached), Impure = dangerous (runs constantly). Use new references to trigger pure pipes!
