...
- [🏛️ What Problem Does It Solve?](#what-problem-does-it-solve)
  ...
  - [The "Glitch-Free" State](#the-glitch-free-state)
- [🔬 Deep Dive: Important Functions](#deep-dive-important-functions)
  - [1. `toSignal(observable$, options)`](#1-tosignalobservable-options)
...
- [2. 🚀 Step-by-Step Implementation](#2--step-by-step-implementation)
  - [Step 1: Create Signal from Control](#step-1-create-signal-from-control)
  - [Step 2: Derive State](#step-2-derive-state)
  - [Step 3: consume in Template](#step-3-consume-in-template)
- [3. 🌍 Real World Use Cases](#3--real-world-use-cases)
- [7. ❓ Interview & Concept Questions](#7--interview--concept-questions)
  - [Q1: Why do we need `initialValue` in `toSignal(valueChanges)`?](#q1-why-do-we-need-initialvalue-in-tosignalvaluechanges)
  - [Q2: Can I bind `[formControl]` directly to a Signal?](#q2-can-i-bind-formcontrol-directly-to-a-signal)
  - [Q3: Difference between `AsyncPipe` and `toSignal`?](#q3-difference-between-asyncpipe-and-tosignal)
  - [Q4: When does `effect()` run?](#q4-when-does-effect-run)
  - [Q5: Is `toSignal` a memory leak?](#q5-is-tosignal-a-memory-leak)
  - [Q6: What happens if you call `toSignal()` outside of an injection context?](#q6-what-happens-if-you-call-tosignal-outside-of-an-injection-context)
  - [Q7: SCENARIO: Your `toSignal(valueChanges)` shows `undefined` before the user types. How do you fix it?](#q7-scenario-your-tosignalvaluechanges-shows-undefined-before-the-user-types-how-do-you-fix-it)
  - [Q8: Can you use `computed()` to combine multiple form control signals?](#q8-can-you-use-computed-to-combine-multiple-form-control-signals)
  - [Q9: What is the difference between `computed()` and `effect()`?](#q9-what-is-the-difference-between-computed-and-effect)
  - [Q10: SCENARIO: You want to save form data to localStorage whenever it changes. Which should you use: `computed()` or `effect()`?](#q10-scenario-you-want-to-save-form-data-to-localstorage-whenever-it-changes-which-should-you-use-computed-or-effect)
  - [Q11: How do you convert `statusChanges` to a Signal?](#q11-how-do-you-convert-statuschanges-to-a-signal)
  - [Q12: Can `effect()` cause infinite loops with forms?](#q12-can-effect-cause-infinite-loops-with-forms)
  - [Q13: What is `requireSync` option in `toSignal`?](#q13-what-is-requiresync-option-in-tosignal)
  - [Q14: SCENARIO: You have a search form and want to debounce before converting to Signal. What's the pattern?](#q14-scenario-you-have-a-search-form-and-want-to-debounce-before-converting-to-signal-whats-the-pattern)
  - [Q15: How do you handle errors in an Observable before converting to Signal?](#q15-how-do-you-handle-errors-in-an-observable-before-converting-to-signal)
  - [Q16: What is `toObservable()` and when would you use it?](#q16-what-is-toobservable-and-when-would-you-use-it)
  - [Q17: SCENARIO: Your computed signal depends on both form value AND a route parameter. How do you structure this?](#q17-scenario-your-computed-signal-depends-on-both-form-value-and-a-route-parameter-how-do-you-structure-this)
  - [Q18: Does `effect()` run during SSR (Server-Side Rendering)?](#q18-does-effect-run-during-ssr-server-side-rendering)
  - [Q19: SCENARIO: You need to call an API whenever a form field changes, but only after validation passes. What's the approach?](#q19-scenario-you-need-to-call-an-api-whenever-a-form-field-changes-but-only-after-validation-passes-whats-the-approach)
  - [Q20: What's the advantage of using Signals over `async` pipe for derived form state?](#q20-whats-the-advantage-of-using-signals-over-async-pipe-for-derived-form-state)
