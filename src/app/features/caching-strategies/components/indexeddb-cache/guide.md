...
## 📋 Table of Contents
- [1. 🔍 Caching Interceptor](#1--caching-interceptor)
- [2. 🚀 Advanced: Selective Caching](#2--advanced-selective-caching)
- [3. ❓ Interview Questions](#3--interview-questions)
  - [Basic Questions](#basic-questions)
    - [Q1: Why use interceptor for caching?](#q1-why-use-interceptor-for-caching)
    - [Q2: Why only cache GET requests?](#q2-why-only-cache-get-requests)
  - [Scenario-Based Questions](#scenario-based-questions)
    - [Scenario: Cache with Headers](#scenario-cache-with-headers)
  - [📦 Data Flow Summary (Visual Box Diagram)](#data-flow-summary-visual-box-diagram)
- [🚗 Highway Checkpoint Analogy (Easy to Remember!)](#highway-checkpoint-analogy-easy-to-remember)
...
```
┌─────────────────────────────────────────────────────────────┐
│  HTTP INTERCEPTOR CACHING: AUTOMATIC FOR ALL REQUESTS       │
│                                                             │
│   INTERCEPTOR FLOW:                                         │
│   ┌───────────────────────────────────────────────────────┐ │
...
│   │   // Check cache                                      │ │
│   │   const cached = cache.get(req.url);                  │ │
│   │   if (cached) return of(cached); // Cache HIT! ⚡     │ │
│   │                                                       │ │
...
│   SELECTIVE CACHING:                                        │
│   ┌───────────────────────────────────────────────────────┐ │
│   │ const CACHEABLE = ['/api/config', '/api/categories']; │ │
│   │ if (!CACHEABLE.includes(req.url)) return next(req);   │ │
│   └───────────────────────────────────────────────────────┘ │
│                                                             │
│   ⚠️ Only cache GET (POST/PUT/DELETE modify data!)         │
└─────────────────────────────────────────────────────────────┘
```

> **Key Takeaway**: Interceptor caches ALL GET requests automatically. Check cache first, store on miss. Only GET = safe to cache!
