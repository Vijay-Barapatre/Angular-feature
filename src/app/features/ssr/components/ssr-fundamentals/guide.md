...
- [🏗️ Angular Universal Architecture](#angular-universal-architecture)
- [💻 Setting Up SSR](#setting-up-ssr)
  - [Step 1: Add Angular SSR Package](#step-1-add-angular-ssr-package)
  - [Step 2: Project Structure After SSR Setup](#step-2-project-structure-after-ssr-setup)
  - [Step 3: Server Configuration (server.ts)](#step-3-server-configuration-serverts)
  - [Step 4: Server App Configuration](#step-4-server-app-configuration)
- [🔄 SSR Request Lifecycle](#ssr-request-lifecycle)
  - [📦 Data Flow Summary (Visual Box Diagram)](#data-flow-summary-visual-box-diagram)
- [📋 When to Use SSR](#when-to-use-ssr)
  - [✅ Good Use Cases](#good-use-cases)
  - [❌ When SSR May Not Help](#when-ssr-may-not-help)
- [🧪 Running SSR Locally](#running-ssr-locally)
  - [Development Mode](#development-mode)
- [⚠️ Common Pitfalls](#common-pitfalls)
  - [1. Browser-Only APIs](#1-browser-only-apis)
  - [2. Timeouts and Subscriptions](#2-timeouts-and-subscriptions)
...
│   ┌───────────────────────────────────────────────────────┐ │
│   │ Request → Empty HTML → Download JS → Render → See 🐢  │ │
│   │ User waits for JavaScript before seeing anything      │ │
│   └───────────────────────────────────────────────────────┘ │
│                                                             │
│   SERVER-SIDE RENDERING (Angular Universal):                │
│   ┌───────────────────────────────────────────────────────┐ │
│   │ Request → Server renders HTML → Full page → See 🚀    │ │
│   │       → JS hydrates → Interactive                     │ │
│   │ User sees content IMMEDIATELY (SEO-friendly!)         │ │
│   └───────────────────────────────────────────────────────┘ │
│                                                             │
│   SETUP:                                                    │
│   ┌───────────────────────────────────────────────────────┐ │
│   │ ng add @angular/ssr                                   │ │
│   │ Creates: server.ts, app.config.server.ts              │ │
│   │                                                       │ │
│   │ RUN: ng build && npm run serve:ssr:app-name           │ │
│   └───────────────────────────────────────────────────────┘ │
│                                                             │
│   ⚠️ GOTCHA: window/document don't exist on server!        │
│      Use isPlatformBrowser() to check                       │
└─────────────────────────────────────────────────────────────┘
```

> **Key Takeaway**: SSR = server renders HTML before sending. Use for SEO + fast first paint. Check platform before using browser APIs!
...
- [Platform Detection](../use-case-4/guide.md) - Browser vs Server
- [Prerendering](../use-case-6/guide.md) - Static generation
